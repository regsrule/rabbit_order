...
std::vector<std::vector<std::vector<int>>> store_partition_rabbit(adjacency_list adj, bool store_affinity) {
  ...
  std::cerr << "Detecting communities rabbit ...\n";
  ...
  std::vector<std::vector<int>> scomm(final_nb);
  std::vector<std::vector<int>> adj_graph(g.n());
  std::vector<std::vector<int>> comm_affinity(final_nb);

  
  //std::cerr << "g.es.size() = " << g.es.size()<< std::endl;
  for (int i=0 ; i < g.n() ; i++){
	  //std::cerr << i << " " << renumber[c[i]] << "\n";
	  scomm[renumber[c[i]]].push_back(i);
    if(store_affinity)
    {
	    std::vector<std::pair<vint,float> > neig = adj[i];
		adj_graph[i].resize(neig.size());
	    //std::cout << i <<"("<< neig.size()<< "):" ;
	if(comm_affinity[renumber[c[i]]].size() == 0)
		comm_affinity[renumber[c[i]]].resize(final_nb,0);
	for(unsigned int k = 0; k < neig.size(); k++) {
		comm_affinity[renumber[c[i]]][renumber[c[neig[k].first]]] += 1;
		adj_graph[i][k] = neig[k].first;
		//std::cout << neig[k].first << "\t";
	}
	//std::cout << std::endl;
    }
  }
  ...
  result.reserve(3);
  result.push_back(scomm);
  result.push_back(comm_affinity);
  result.push_back(adj_graph);
  return result;
}
...
std::vector<std::vector<int>> reorder_set_graph(adjacency_list adj) {
  auto _adj = adj;  // copy `adj` because it is used for setting the new graph
  std::cerr << "Generating a permutation...\n";
  const double tstart = rabbit_order::now_sec();
  //--------------------------------------------
  const auto g = rabbit_order::aggregate(std::move(_adj));
  const auto p = rabbit_order::compute_perm(g);
  //--------------------------------------------
  std::cerr << "Runtime for permutation generation [sec]: "
            << rabbit_order::now_sec() - tstart << std::endl;
  ...
  for (int i=0 ; i < g.n() ; i++){
	    std::vector<std::pair<vint,float> > neig = adj[i];
		adj_graph[p[i]].resize(neig.size());
	    //std::cout << i <<"("<< neig.size()<< "):" ;
	for(unsigned int k = 0; k < neig.size(); k++) {
		adj_graph[p[i]][k] = p[neig[k].first];
		//std::cout << neig[k].first << "\t";
	}
  }
  #pragma omp barrier
